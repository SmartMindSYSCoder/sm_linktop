...
public class HcService extends Service   {
    ...
    public final static int SDK_ERROR = -1;
    public final static int DATA_QUERY_CONFIRM_ECG_MODULE_EXIST = 1;
    public final static int DATA_QUERY_CONFIRM_BG_MODULE_EXIST = 2;

    public final static int DATA_QUERY_CONFIRM_AUTH_CHECK = 3;

    public final static int DATA_QUERY_ID_AND_KEY = 4;
    public final static int DATA_QUERY_SOFTWARE_VER = 5;
    public final static int DATA_QUERY_HARDWARE_VER = 6;
    public final static int DATA_QUERY_FIRMWARE_VER = 7;
    public final static int DATA_QUERY_BATTERY_INFO = 8;
    ...
    public void onCreate() {
        ...
        mBleDevManager.initHC(this, e -> {
            if (mHandler != null) {
                Message.obtain(mHandler, SDK_ERROR, e);
            }
        });
        super.onCreate();
    }
...
//        Log.d("bluetoothConnection","**********************   state :"+state);
        sendEvent(false);
        ...
        //    UToast.show(getBaseContext(), "Mobile phone not support BLE!");
            ...
            if (bluetoothManager != null) {
                ...
                if (mAdapter.isEnabled()) {
                  ...
                  //  sendEvent(false,"BLE_OPENED_AND_DISCONNECT");
                } else {
                    setBleState(BluetoothState.BLE_CLOSED);
                   // sendEvent(false,"BLE_CLOSED");
    ...
    public void connect(BluetoothDevice device) {
     ...
     //   sendEvent(false,"BLE_CONNECTING_DEVICE");
...
//                        mBleDevManager.getEcgTask().setModuleExist(false);
            ...
            public void onServicesDiscovered(BluetoothGatt gatt, int status) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                  ...
                  //  sendEvent(isEnabled,  isEnabled ?"BLE_CONNECTED_DEVICE" :"BLE_OPENED_AND_DISCONNECT");
    ...
    public void dataQuery(int step) {
        switch (step) {
            case DATA_QUERY_CONFIRM_ECG_MODULE_EXIST:
                ...
                mBleDevManager.getEcgTask().checkModuleExist(() -> {
                    try {
                        Thread.sleep(100L);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
...
//                    dataQuery(DATA_QUERY_ID_AND_KEY);
                    dataQuery(DATA_QUERY_CONFIRM_AUTH_CHECK);
                });
                break;
            case DATA_QUERY_CONFIRM_AUTH_CHECK:
                /*
                 ...
                 */
                if (mBleDevManager.getCommunicate().needDeviceAuthCheck()) {
                    mBleDevManager.getCommunicate().authCheck(new AuthCheckResult() {
                        @Override
...
//                }
    ...
    public void sendEvent(boolean isMeasuring){
            ...
            activity.runOnUiThread(() -> {
                if (events != null) {
                    ...
                    try {
                        ...
                        inputObject.put("isConnected", isConnected);
                        inputObject.put("connectStateMsg", isConnected ? "Connected" :"Disconnected");
                        inputObject.put("isMeasuring", isMeasuring);
